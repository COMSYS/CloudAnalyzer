...
public class AppServiceRegionDataAmountFragmentGeneralDown extends StatsDataAmountAbstract
{
    @Override
    public void onAttach(Context context)
    {
        ...
        sql_cloud = "SELECT day, year, SUM(bytes) FROM aggregation_view WHERE serviceID = ? AND appID = ? AND regionID = ? AND direction = 0 " + trafficPropertyILP + " GROUP BY day, year ORDER BY year, day";
